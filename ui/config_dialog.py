...
class BatchConfigDialog(QDialog):
    ...
    def init_ui(self):
        """初始化界面"""
        self.setWindowTitle(f'批量配置 - {len(self.nodes)} 个节点')
        self.setFixedSize(400, 250)

        layout = QVBoxLayout()

        # 信息标签
        info_label = QLabel(f'将对以下 {len(self.nodes)} 个节点进行批量配置:')
        ...
        self.combo_entity = QComboBox()
        self.combo_entity.setEditable(True)
        self.combo_entity.addItems(['', '01', '02', '03', '98', '99'])
        form_layout.addRow('发运主体:', self.combo_entity)

        # 发运方式
        self.combo_method = QComboBox()
        self.combo_method.addItems(['', 'A-散装', 'B-打捆', 'C-装箱', 'D-特殊'])
        form_layout.addRow('发运方式:', self.combo_method)
    ...
    def accept(self):
        """确认保存"""
        method_display = self.combo_method.currentText()
        method_code = method_display[0] if method_display and '-' in method_display else ''

        entity_text = self.combo_entity.currentText()
        entity_code = entity_text.split('-')[0] if entity_text and '-' in entity_text else entity_text

        for node in self.nodes:
            # 业务逻辑：设置发运配置时，如果节点是"展开"状态，自动改为"否"
            if entity_code or method_code:
